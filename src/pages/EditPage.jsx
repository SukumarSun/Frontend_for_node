...
const EditPage=()=>{
    ...
    return (
        ...
        <h2 className="font-semibold text-2xl mb-4 block text-center">Update Product -{product.name}</h2>
        {isLoading?("Loading"):
        <>
          <form onSubmit={updateProduct}>
            <div className="space-y-2">
                ...
                <div>
                    {!isLoading && (<button className="block w-full mt-6 bg-blue-700 text-white rounded-sm p-4 font-bold hover:bg-blue-600 hover:cursor-pointer">Update</button>)}
                 </div>
            </div>
        </form>
        </>}
